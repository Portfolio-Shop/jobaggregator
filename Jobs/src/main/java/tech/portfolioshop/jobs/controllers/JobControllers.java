...
public class JobControllers {
    ...
    @GetMapping(params = {"query", "location"})
    public ResponseEntity<List<JobResponse>> findJobsByQuery(
        ...
        if(token != null && !token.isEmpty()){
            try{
                String userId = Jwts.parser()
                        ...
                        .getBody()
                        .getSubject();
                jobs = jobsService.findJobByQuery(query, location, userId);
            } catch (Exception ignored) {
                jobs = jobsService.findJobByQuery(query, location);
            }
        } else {
            jobs = jobsService.findJobByQuery(query, location);
        }
    ...
    @GetMapping(headers = {HttpHeaders.AUTHORIZATION}, params = {})
